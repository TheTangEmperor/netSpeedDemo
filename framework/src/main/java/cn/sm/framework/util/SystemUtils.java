...
public final class SystemUtils {
     ...
     * @param phoneNumber
     */
    public static void openPhone(Activity activity, String phoneNumber) {
        if (activity != null && !TextUtils.isEmpty(phoneNumber)) {
            if (ActivityCompat.checkSelfPermission(activity, Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED) {
                JLog.d("not permission");
     ...
     * @return
     */
    public static String pickImageUriToPath(Activity activity, Uri uri){
...
// 从数据视图中获取已选择图片的路径
    ...
    public static void openAppInfoPage(Context context){
        ...
        try{
            ...
            JLog.e("打开本应用设置界面失败： " + e.getMessage());
            try{
